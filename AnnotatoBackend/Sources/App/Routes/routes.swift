...
func routes(_ app: Application) throws {
    ...
    app.get("test") { _ -> String in
        let testDocument = Document(name: "Test Document", ownerId: "user", baseFileUrl: "path/to/document")
        let testDocumentEntity = DocumentEntity.fromModel(testDocument)
        _ = testDocumentEntity.save(on: app.db)

        print(testDocumentEntity)

        let testAnnotation = Annotation(origin: .zero, width: .infinity, ownerId: "user", documentId: testDocumentEntity.id!)
        ...
        let testAnnotationText = AnnotationText(type: .markdown, content: "Content", height: 100, annotationId: testAnnotationEntity.id!)
